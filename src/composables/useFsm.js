...
export default function useFSM(record) {
  ...
  const transitions = computed(() => {
    return Object.entries(record.http.data?.links?.transitions || {}).map(
      ([code, url]) => ({
        url,
        code: code,
        label: t(`action.${code}`),
  ...
  function makeTransition(transition) {
    if (transition.code === "request_approval" && hasNoDOI.value) {
      $q.dialog({
        ...
        if (
          validity !== false ||
          ["request_changes", "delete_draft"].includes(transition.code)
        ) {
          ...
          $q.dialog({
            ...
            message: `${t("message.recordNotValid")}. ${t(
              "message.doYRlyWnt"
  ...
  async function _makeTransition(transition, force, data = null) {
    ...
    try {
      ...
      if (transition.code === "delete_draft") {
        await router.replace({ name: "community-datasets" });
        $q.notify({
      ...
      $q.dialog({
        ...
        message: `${t("error.stateChangeMessage")}: ${e} ${JSON.stringify(
          e.response.data
        )}`,
        cancel: false,
        persistent: true,
